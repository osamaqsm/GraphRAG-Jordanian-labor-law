...
def _assert_strict_required(node: object) -> None:
...
def _assert_prompt_contract() -> None:
    ...
    required_clauses = (
        ...
        "the user asks for the next step in a multi-stage legal procedure",
        "Do not add any fact, actor, institution, remedy, condition, procedure",
        "Each retrieval query must contain approximately 5 to 20 Arabic words.",
        "One atomic issue normally means:",
        "maximum_articles = 1",
        "Confidence represents confidence in the routing decision",
    )
    for clause in required_clauses:
        assert clause in prompt, clause

    scope_index = prompt.index("STEP 1 — SCOPE")
    sufficiency_index = prompt.index("STEP 2 — FACTUAL SUFFICIENCY")
    retrieval_index = prompt.index("STEP 3 — RETRIEVAL PLANNING")
    assert scope_index < sufficiency_index < retrieval_index

    assert "must not select, infer, mention, or guess article numbers" in prompt
    assert '"وفق قانون العمل الأردني"' in prompt
...
def main() -> None:
    ...
    strict_schema = planner._strict_response_schema()
    _assert_strict_required(strict_schema)
    assert "actors" in strict_schema["$defs"]["AtomicLegalIssue"]["required"]
    assert "atomic_issues" in strict_schema["required"]
    ...
    print("Stage 7.7-B prompt-only checks passed.")
    print("Prompt contract: scope -> sufficiency -> retrieval")
    print("Strict OpenAI schema: every object property is required")
