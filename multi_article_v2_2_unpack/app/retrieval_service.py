...
class RetrievalService:
    ...
    def _llm_article_selection(
        ...
        *,
        ...
        seed_ranks = min(4, max_issue_candidates)
        ...
        if issue_pairs:
            for issue_index, (label, query) in enumerate(
                ...
                # Build a focused verification catalogue from the issue's own
                # candidates, their nearby statutory provisions, and any support
                # proposed by the global reranker. The verifier is allowed to
                # say uncovered, unlike the old single-issue selector.
                should_verify = (
                    not support_numbers
                    or (
                        bool(values)
                        and any(number != int(values[0]) for number in support_numbers)
                    )
                )

                if should_verify:
                    verification_numbers: list[int] = []
                    for number in [*values, *support_numbers]:
                        number = int(number)
                    ...
                    for distance in range(1, neighbor_radius + 1):
                        for seed_number in seed_values:
                            for number in (seed_number - distance, seed_number + distance):
                                if number in full_catalog and number not in verification_numbers:
                                    verification_numbers.append(number)

                    repair_catalog: list[CatalogArticle] = []
                    for candidate_number in verification_numbers[:12]:
                        hit = ranked_by_number.get(int(candidate_number))
                        if hit is None:
                            catalog_hit = full_catalog.get(int(candidate_number))
                            hit = replace(catalog_hit) if catalog_hit is not None else None
                        if hit is None:
                            continue
                    ...
                    if repair_catalog:
                        ...
                        if verified is not None:
                            if verified.covered:
                                ...
                                source = "targeted_support_verification"
                            else:
                                support_numbers = []
                                source = "uncovered"
                ...
                coverage_debug.append(
                    {
                        ...
                        "support_article": (support_numbers[0] if support_numbers else None),
