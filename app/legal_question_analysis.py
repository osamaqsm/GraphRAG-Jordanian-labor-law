...
class LegalQuestionAnalysis:
    ...
    planner_issue_labels: tuple[str, ...] = ()
    ...
    def bm25_query(self) -> str:
        normalized_original = normalize_arabic(self.original_question)
        additions: list[str] = []
        ...
        for term in (
            ...
            *self.planner_queries,
            ...
            if not normalized_term:
                continue
            if (
                term in self.planner_queries
                or normalized_term not in normalized_original
    ...
    @property
    def reranker_question(self) -> str:
        if not self.planner_queries:
            return self.original_question

        issue_lines = [
            f"- {label}: {query}"
            for label, query in zip(
                self.planner_issue_labels,
                self.planner_queries,
                strict=False,
            )
        ]
        ...
        return "\n".join(
            [
                ...
                "Structured pre-retrieval issue plan:",
                *issue_lines,
            ]
        )
